...
def MovingAverageCrossStrategy(symbol, short_window, long_window, moving_avg, data, print_out=True, cc=0.0, ccc=0.0):
    """
    ...
    # Create a short simple exponential average (short_ema) & create a long exponential moving average (long_ema) column
        ...
        stock_df[short_window_col] = ta.WMA(stock_df['Close Price'], short_window)
        stock_df[long_window_col] = ta.WMA(stock_df['Close Price'], long_window)            
    ...
    if print_out == True:
        try:
            ...
            plt.title(
                f"{symbol} - {str(moving_avg)} Crossover",
                fontsize=30,
                fontweight="bold",
            )

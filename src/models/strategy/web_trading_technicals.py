...
class Trading_Technicals(object):
    ...
    def trading_technicals(self):
        ...
        df = df.fillna(
            method="ffill"
        )  # fills any missing day's data with previous day's
        ...
        MAX_STOCK_PURCHASE = 2500  # maximum spent on each buy signal
        BROKERAGE_FEE = 9.50  # Selfwealth brokerage fee
        PROFIT_THRESHOLD = 1.2
        COOLDOWN_PERIOD = (
            30  # number of elapsed days before another buy can be executed
        )
        ...
        ax2.bar(
            ...
            histogram_y,
            color=[
                "g" if histogram_y[i] > 0 else "r" for i in range(0, len(histogram_y))
            ],
            width=1,
            label="Histogram",
        )
        ...
        crossovers["Crossover"] = np.where(
            crossovers["position"] == crossovers["pre-position"], False, True
        )
        ...
        crossovers = crossovers.drop(
            ["position", "pre-position", "Crossover", "index"], axis=1
        )

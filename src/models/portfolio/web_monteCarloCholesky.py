...
class MonteCarloCholesky(object):
    ...
    def __init__(self, date_report):
        self.report_date = str(date_report)[:10]
    ...
    def import_stock_data(self, tickers):
        return pd.DataFrame(yf.download(tickers, start="2019-01-01", end=self.report_date)["Adj Close"])
    ...
    def probs_find(self, predicted, higherthan, ticker=None, on="value"):
        """
        This function calculated the probability of a stock being above a certain threshhold, which can be defined
            as a value (final stock price) or return rate (percentage change)
            
        >>> Input:
            1. predicted: dataframe with all the predicted prices (days and simulations)
            2. higherthan: specified threshhold to which compute the probability (ex. 0 on return will compute the probability 
                of at least breakeven)
            3. on: 'return' or 'value', the return of the stock or the final value of stock for every simulation 
                over the time specified
    ...
    def montecarlo_cholesky(self, tickers, days, iterations, show_hist=True, show_line=True, show_stats=True):
        #   Get tickers and number of tickers involved in this portfolio
        data = self.import_stock_data(tickers)        
        ...
        for s in range(len(ticks)):
            ...
            if show_stats == True:
                ...
                text1 = (
                    (pd.DataFrame(price_list).iloc[-1].mean() - price_list[0, 1]) * 100
                ) / pd.DataFrame(price_list).iloc[-1].mean()
                text2 = self.probs_find(pd.DataFrame(price_list), 0, on="return")
                st.write(f"__Return:__ {round(text1, 2)}%")
                st.write(f"__Probability of Breakeven:__ {text2}")
                st.write(f"{'__'*25}")
    ...
    def beta_sharpe(self, tickers, data, mark_ticker="^GSPC", riskfree=0.0288, riskpremium=0.0712):
        dd, mark_ret = self.market_data_combination(tickers, data, mark_ticker)
        ...
        #   Annualized variance of market
        #   Now we have our betas!
    ...
    def market_data_combination(self, data, mark_ticker="^GSPC"):
        market_data = self.import_stock_data(mark_ticker)
        market_rets = self.log_returns(market_data).dropna()
    ...
    def mcc_sharpe(self, riskfree, data_train, trials=1347, plot_eff=False): 
        tickers = list(data_train.columns)
        data = data_train

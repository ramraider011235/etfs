...
def get_strong_undervalued_stocks():
    screener_df = pd.DataFrame(
        _raw_get_daily_info(
            "https://finance.yahoo.com/screener/predefined/strong_undervalued_stocks?ncid=dcm_320339102_490172245_127172993?offset=0&count=100"
        )
...
def get_undervalued_large_caps():
    screener_df = pd.DataFrame(
        _raw_get_daily_info(
            "https://finance.yahoo.com/screener/predefined/undervalued_large_caps?offset=0&count=145"
        )
    ).sort_values("% Change", ascending=False)
    rank = []
    [rank.append(x) for x in range(1, len(screener_df) + 1)]
...
def get_undervalued_growth_stocks():
    screener_df = pd.DataFrame(
        _raw_get_daily_info(
            "https://finance.yahoo.com/screener/predefined/undervalued_growth_stocks?offset=0&count=250"
        )
    ).sort_values("% Change", ascending=False)
    rank = []
    [rank.append(x) for x in range(1, len(screener_df) + 1)]
